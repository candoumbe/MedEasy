...
export class TokenService {
    ...
    public async refresh(username: string, token: MedEasy.DTO.BearerTokenInfo): Promise<Option<Promise<MedEasy.DTO.BearerTokenInfo>, Promise<{ [key: string]: string | Array<string> }>>> {
        ...
        let optionalResult: Option<Promise<MedEasy.DTO.BearerTokenInfo>, Promise<{ [key: string]: string | Array<string> }>>;

        if (response.ok) {
            optionalResult = Option.some<Promise<MedEasy.DTO.BearerTokenInfo>, Promise<{ [key: string]: string | Array<string> }>>(await response.json() as Promise<MedEasy.DTO.BearerTokenInfo>)
        } else {
            optionalResult = Option.none<Promise<MedEasy.DTO.BearerTokenInfo>, Promise<{ [key: string]: string | Array<string> }>>(await response.json() as Promise<{ [key: string]: string | Array<string> }>)
        }
