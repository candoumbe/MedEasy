    ...
    where TDataStore : DbContext
{
...
#if NET6_0_OR_GREATER
    public DataStoreSeedInitializerAsync(IHostEnvironment hostingEnvironment, ILogger<DataStoreSeedInitializerAsync<TDataStore>> logger, TDataStore dataStore, Func<TDataStore, Task> seeder) 
#else
    public DataStoreSeedInitializerAsync(IHostingEnvironment hostingEnvironment, ILogger<DataStoreSeedInitializerAsync<TDataStore>> logger, TDataStore dataStore, Func<TDataStore, Task> seeder) 
#endif
    {
    ...
    public async Task InitializeAsync()
    {
        try
        {
            ...
            await policy.ExecuteAsync(async () => await _seeder.Invoke(_store).ConfigureAwait(false))
