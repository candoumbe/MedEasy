namespace Measures.CQRS.Handlers.Subjects
{
    ...
    /// </summary>
    public class HandleDeleteSubjectInfoByIdCommand : IRequestHandler<DeletePatientInfoByIdCommand, DeleteCommandResult>
    {
        ...
        /// <summary>
        /// Builds a new <see cref="HandleDeleteSubjectInfoByIdCommand"/> instance.
        /// </summary>
        ...
        /// </exception>
        public HandleDeleteSubjectInfoByIdCommand(IUnitOfWorkFactory uowFactory, IExpressionBuilder expressionBuilder, IMediator mediator)
        {
        ...
        public async Task<DeleteCommandResult> Handle(DeletePatientInfoByIdCommand cmd, CancellationToken cancellationToken)
        {
            IEnumerable<Task<bool>> anyMesureTask = new[]{
                    _uowFactory.NewUnitOfWork().Repository<BloodPressure>().AnyAsync(x => x.Subject.Id == cmd.Data, cancellationToken).AsTask(),
                    _uowFactory.NewUnitOfWork().Repository<Temperature>().AnyAsync(x => x.Subject.Id == cmd.Data, cancellationToken).AsTask(),
            };
            ...
            else if (!await uow.Repository<Subject>()
            ...
            {
                uow.Repository<Subject>().Delete(x => x.Id == cmd.Data);
