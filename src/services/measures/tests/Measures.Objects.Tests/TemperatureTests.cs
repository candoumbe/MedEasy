...
namespace Measures.Objects.UnitTests
{
    ...
    [Feature(nameof(Temperature))]
    public class TemperatureTests
    {
        private readonly ITestOutputHelper _outputHelper;
        private readonly static Faker _faker;

        static TemperatureTests()
        {
            _faker = new Faker();
        }

        public TemperatureTests(ITestOutputHelper outputHelper)
        {
        ...
        public static IEnumerable<object[]> CtorArgumentOutOfRangeExceptionCases
        {
            get
            {
                ...
                yield return new object[] { Guid.NewGuid(), Guid.NewGuid(), 14.January(1985), _faker.Random.Float(min: float.MinValue, max:Temperature.LowestKnownTemperature), "value is lower that lowest measurable temperature." };
            }
        }
        ...
        public void Ctor_throws_ArgumentOutOfRangeException_when_argument_is_out_of_expected_range(Guid id, Guid patientId, DateTime dateOfMeasure, float value, string reason)
        {
            ...
            // Act
            Action ctorWithInvalidArgument = () => new Temperature(id, patientId, dateOfMeasure, value);
        ...
        {
            // Act
            Temperature measure = new Temperature(Guid.NewGuid(), Guid.NewGuid(), 13.January(2009), 0);
        ...
        public static IEnumerable<object[]> ChangeValueInvalidCases
        {
            get
            {
                Temperature measure = new Temperature(Guid.NewGuid(), Guid.NewGuid(), 13.January(2009), 8);

                yield return new object[]
                {
                    measure,
                    float.NegativeInfinity,
                    "new value is negative infinity"
                };
        ...
        public void ChangeValue_with_incorrect_value_throws_ArgumentOutOfRangeException(Temperature measure, float newValue, string reason)
        {
            // Act
        ...
        public void ChangeValue_sets_Value()
        {
            // Arrange
            Temperature measure = new Temperature(Guid.NewGuid(), Guid.NewGuid(), 13.January(2009), 0);
            float newValue = _faker.Random.Float(min: Temperature.LowestKnownTemperature, max: float.MaxValue);

            _outputHelper.WriteLine($"Value to set : {newValue}");
