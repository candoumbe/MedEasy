...
namespace Documents.CQRS.UnitTests.Features.Documents.Handlers
{
    [Feature("Documents")]
    [UnitTest]
    public class HandleGetOneDocumentInfoByIdQueryTests : IDisposable, IClassFixture<SqliteDatabaseFixture>
    {
        private IUnitOfWorkFactory _uowFactory;
        private HandleGetOneDocumentInfoByIdQuery _sut;

        public HandleGetOneDocumentInfoByIdQueryTests(ITestOutputHelper outputHelper, SqliteDatabaseFixture database)
        {
        ...
        {
            // Arrange
            GetOneDocumentInfoByIdQuery request = new GetOneDocumentInfoByIdQuery(Guid.NewGuid());

            // Act
            Option<DocumentInfo> optionalDocument = await _sut.Handle(request, cancellationToken: default)
                .ConfigureAwait(false);
